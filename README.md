...
对线上环境进行资源优化可以带来节省大量的基础设施成本，优化之前需要首先对资源使用情况进行监控。为了追求性能和稳定性，Java程序会通过把参数`-Xmx`和`-Xms`设置成一样的值来避免了频繁的回收和重新分配堆内存，这导致了系统层面的内存监控无法得知Java程序的内存实际占用情况，容易造成了资源的浪费，本文介绍了一种低成本的对 Java程序内存实际使用情况进行监控的解决方案，同时就如何优化资源管控提供了一些看法。
